...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match('/^[a-zA-Z0-9._-]{3,50}$/', $newValues['username'])) {
        $errors['username'] = 'Username iwe na herufi 3-50: letters, numbers, dot, underscore au hyphen.';
    }
    if (mb_strlen($newValues['display_name']) < 2 || mb_strlen($newValues['display_name']) > 80) {
        $errors['display_name'] = 'Display name iwe na herufi 2-80.';
    }
    if (!in_array($newValues['role'], ['owner', 'admin'], true)) {
        $errors['role'] = 'Role si sahihi.';
    }
    if (strlen($password) < 12 || strlen($password) > 200) {
        $errors['password'] = 'Temporary password iwe na herufi 12-200.';
    }
    if (!$errors) {
        try {
            $now = et_utc_now();
...
SQL);
            ...
            et_flash('success', 'Admin mpya ametengenezwa. Mpe username na temporary password kwa njia salama.');
            et_redirect('./');
        } catch (PDOException $exception) {
            $errors['form'] = str_contains(strtolower($exception->getMessage()), 'unique') ? 'Username hiyo imetumika.' : 'Admin hakuweza kutengenezwa.';
        }
    }
}
...
?>
<div class="admin-page-heading"><div><p>Owner anaweza kuongeza, kubadilisha role, kusimamisha au kuondoa access.</p></div></div>
<?php if (isset($errors['form'])): ?><div class="admin-alert error"><?= et_e($errors['form']) ?></div><?php endif; ?>
<section class="form-section" style="margin-bottom:18px"><h2>Ongeza admin</h2><form method="post" class="admin-form"><input type="hidden" name="csrf_token" value="<?= et_e(et_csrf_token()) ?>"><div class="form-grid">
