...
$parameters = [];
$where = '';
if ($action !== '') { $where = ' WHERE audit_logs.action=:action'; $parameters['action'] = $action; }
$countStatement = $database->prepare('SELECT COUNT(*) FROM audit_logs' . ($action !== '' ? ' WHERE action=:action' : ''));
...
<?php foreach ($logs as $log): ?><tr><td><?= et_e($log['created_at']) ?><small>UTC</small></td><td><?= et_e($log['display_name'] ?: 'System/unknown') ?></td><td><?= et_e(str_replace('_', ' ', $log['action'])) ?></td><td><?= et_e($log['entity_type']) ?><?= $log['entity_id'] ? ' #' . (int) $log['entity_id'] : '' ?></td><td><?= et_e($log['details']) ?></td></tr><?php endforeach; ?>
</tbody></table></div>
